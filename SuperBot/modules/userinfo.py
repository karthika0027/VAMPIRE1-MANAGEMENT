...
import SuperBot.modules.sql.userinfo_sql as sql
...
from SuperBot import telethn as AngelTelethonClient
from SuperBot.__main__ import STATS, TOKEN, USER_INFO
from SuperBot.modules.disable import DisableAbleCommandHandler
from SuperBot.modules.helper_funcs.chat_status import sudo_plus
from SuperBot.modules.helper_funcs.extraction import extract_user
from SuperBot.modules.sql.afk_sql import check_afk_status, is_afk
from SuperBot.modules.sql.global_bans_sql import is_user_gbanned
from SuperBot.modules.sql.users_sql import get_user_num_chats
...
def info(update: Update, context: CallbackContext):
    ...
    if user_id not in [bot.id, 777000, 1955509952]:
            ...
            message.reply_document(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "ʜᴇᴀʟᴛʜ", url="https://t.me/LOGI_CHANNEL/57"
                            ),
                            InlineKeyboardButton(
                                "ᴅɪꜱᴀꜱᴛᴇʀ", url="https://t.me/LOGI_CHANNEL/67"
                            ),
                        ],
                    ]
                ),
...
def set_about_me(update: Update, context: CallbackContext):
    ...
    if user_id in [777000, 1955509952]:
    ...
    if message.reply_to_message:
        ...
        if repl_user_id in [bot.id, 777000, 1955509952] and (user_id in DEV_USERS):
...
def set_about_bio(update: Update, context: CallbackContext):
    ...
    if message.reply_to_message:
        ...
        if user_id in [777000, 1955509952] and sender_id not in DEV_USERS:
